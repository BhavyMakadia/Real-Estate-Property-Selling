...
export default function AdminShowlisting() {
  const fileRef = useRef(null);
  const { currentUser, loading, error } = useSelector((state) => state.user);
  const [file, setFile] = useState(undefined);
  const [filePerc, setFilePerc] = useState(0);
  ...
  return (
    ...
    <div className="pl-60 px-44 mx-auto  bg-rounded-xl">
      ...
      {userListings && userListings.length > 0 && (
        <div className=''>
         ...
         <div className='pb-16'>
          {userListings.map((listing) => (
            <div key={listing._id} className=' bg-gradient-to-r from-orange-200 to-blue-100 border rounded-lg text-center p-3 flex justify-between items-center gap-4 m-20'>
              <Link to={`/listing/${listing._id}`}>
